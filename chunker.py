...
def _build_sections(pages: List[PageData]) -> List[dict]:
    ...
    for page in pages:
        ...
        if page.section_headers:
            # One or more new section headers on this page.
            # For every header except the last, flush the accumulated section
            # and start an empty section under that intermediate header.
            for header in page.section_headers[:-1]:
                ...
                current_text_parts = []
                current_pages = []
                current_page_offsets = []
            # Handle the final (most specific) header: flush whatever preceded
            # it, then open a new section containing this page's content.
            if current_text_parts:
                sections.append(
                    {
                        "header": current_header,
