...
class QueryRequest(BaseModel):
    ...
    def question_must_not_be_whitespace(cls, v: str) -> str:
        # Field(min_length=10) runs on the raw value, so leading/trailing
        # whitespace could satisfy it and then strip down to near-empty.
        # Re-validate the length AFTER stripping so the 10-char minimum is real.
        v = v.strip()
        if not v:
            raise ValueError("Question cannot be empty or only whitespace.")
        if len(v) < 10:
            raise ValueError("Question must be at least 10 non-whitespace characters.")
        return v

    @field_validator("search_mode")
    @classmethod
    def search_mode_must_be_valid(cls, v: Optional[str]) -> Optional[str]:
        if v is None:
            return None

        normalized = v.strip().lower()
        if normalized not in VALID_SEARCH_MODES:
            valid = ", ".join(VALID_SEARCH_MODES)
            raise ValueError(f"search_mode must be one of: {valid}.")
        return normalized
...
# CORS allowlist — set CORS_ALLOWED_ORIGINS in env to a comma-separated list
# of origins (e.g. "https://app.example.com,https://staging.example.com") to
# tighten this in production. Defaults to "*" for the local demo.
app.add_middleware(
    CORSMiddleware,
...
if CORS_ALLOWED_ORIGINS == ["*"]:
    logger.warning(
        "CORS allow_origins is '*' — fine for local/demo, NOT for a public deployment. "
        "Set CORS_ALLOWED_ORIGINS in env to a concrete origin list."
    )
...
def query_endpoint(request: QueryRequest):
    ...
    try:
        ...
        # Retrieval/embedding failure (e.g. the embedding model failed to load
        # at startup, or a FAISS error). Surface as 503 "not ready" rather than
        # leaking a raw 500. The index-not-loaded case is already guarded above.
        logger.error("Retrieval failed (%s): %s", type(exc).__name__, exc)
        raise HTTPException(
            status_code=503,
            detail="Retrieval backend is unavailable (index or embedding model not ready).",
        )
    ...
    sources = trace_response(_answer_for_tracing, results)
...
def ingest_endpoint(
    pdf_directory: Optional[str] = Form(default=None),
    ...
    Args:
        ...
        x_ingest_token: When INGEST_TOKEN is configured server-side, the request
                       must carry the matching value in the X-Ingest-Token header.
                       Without INGEST_TOKEN configured the endpoint is open
                       (development default).
    """
    if INGEST_TOKEN is not None and x_ingest_token != INGEST_TOKEN:
        # Note: this is a coarse shared-secret check, not full auth. For real
        # deployments put the API behind a proper auth layer (reverse proxy,
        # OAuth, etc.) — this guard is the minimum to keep the index-rebuild
        # endpoint from being trivially abusable when exposed.
