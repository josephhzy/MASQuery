...
- context_relevance:  Mean question-context embedding similarity (a similarity
                      proxy, NOT RAGAS context precision / the relevant-fraction).
- answer_faithfulness: Embedding and token-overlap proxy for context groundedness (not an entailment check — token-overlap signal is inflated by domain vocabulary shared across all MAS documents; see answer_faithfulness docstring for implementation details).
...
def context_relevance(question: str, contexts: List[str]) -> float:
    ...
    context embedding, then returns the mean similarity. NOTE: this is a mean
    question-context similarity proxy, not RAGAS context precision/relevance —
    it does not score the relevant-vs-noise fraction of the retrieved chunks.
...
def answer_faithfulness(answer: str, contexts: List[str]) -> float:
    ...
    Returns:
        ...
        Note: `token_score` is upward-biased on regulatory corpora — high-frequency domain
        vocabulary (e.g. 'financial', 'institutions', 'must', 'requirements') inflates overlap
