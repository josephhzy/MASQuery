...
class TestVerifyCitations:
    ...
    def test_false_accept_fabricated_claim_stamps_verified_true(self):
        # TODO(NLI-upgrade): This test DOCUMENTS a known limitation, not correct
        # behaviour.  A citation whose doc+section+page tokens match a real chunk
        # scores >= CITATION_MATCH_THRESHOLD and is stamped verified=True even when
        # the LLM's claim is not supported by the chunk text.  The fix is to add a
        # cross-encoder NLI step in _find_matching_result; see docs/VERIFICATION.md
        ...
        # The verifier has no visibility into the claim text; it only matched metadata.
        results = [
            RetrievalResult(
                chunk_id="trm_p12_c0",
                text="[Section: 5.2 Access Control] Multi-factor authentication requirements...",
        ...
        refs = verify_citations(citations, results)
        assert len(refs) == 1
        # This assertion documents the false-accept: verified is True even though the
        # claim ("Banks may store passwords in plaintext") is not in the chunk text.
        assert refs[0].verified is True
        assert refs[0].chunk_id == "trm_p12_c0"
