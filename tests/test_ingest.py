...
class TestSectionHeaderDetection:
    ...
    def test_no_headers_on_uniform_text(self):
        """When all text is uniform size and non-bold, no section headers are detected."""
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            pages = extract_pdf(pdf_path)
            assert len(pages) >= 1
            # The body is inserted at a single non-bold font size, so the header
            # heuristic (size > 1.15 * median, OR bold) cannot fire: nothing
            # exceeds the median and there is no bold run. Headers must be empty.
            assert pages[0].section_headers == []
