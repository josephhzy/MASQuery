...
from chunker import Chunk, _get_pages_for_chunk, chunk_document, count_tokens
...
class TestChunkDocument:
    ...
    def test_table_only_page_still_produces_chunk(self):
        """A page whose body text is empty but which has a (sufficiently large)
        table must still index the table content.

        A table below CHUNK_MIN_SIZE is intentionally dropped (see
        test_minimum_chunk_size_enforced), so this uses a table large enough to
        clear the floor — the genuinely empty-body case worth guaranteeing.
        """
        rows = "\n".join(f"Requirement {i} | Priority {i} | Owner Team {i}" for i in range(1, 16))
        table = f"Requirement | Priority | Owner\n{rows}"
        pages = [
            PageData(
                doc_name="TableOnly",
                page_number=1,
        ...
        chunks = chunk_document(pages)
        assert chunks, "Expected a chunk for a table-only page with a large table"
        assert any("Requirement 1 | Priority 1" in c.text for c in chunks)


class TestGetPagesForChunkFallback:
    def test_fallback_when_anchor_not_found(self):
        """When the anchor is not present in section_text, all section pages are returned."""
        section_text = "Page one content. " * 40 + "\n\n" + "Page two content. " * 40
        page_offsets = [(0, 1), (len("Page one content. " * 40) + 2, 2)]
        # chunk_text whose first 80 stripped chars do not appear in section_text
        chunk_text = "Completely different text that was never in the section at all. " * 2
        result = _get_pages_for_chunk(chunk_text, section_text, page_offsets)
        # Fallback must return ALL section pages, not a subset
        assert result == [1, 2], f"Expected full section page range [1, 2], got {result}"
