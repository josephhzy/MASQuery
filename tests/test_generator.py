...
class TestComputeConfidence:
    ...
    def test_non_cosine_high_score_is_medium(self):
        """Non-cosine paths always return medium regardless of score magnitude."""
        assert compute_confidence([0.80, 0.70], score_is_cosine=False) == "medium"

    def test_non_cosine_low_score_is_medium(self):
        """Non-cosine paths return medium even for scores that would be low on cosine scale."""
        assert compute_confidence([0.10], score_is_cosine=False) == "medium"
...
class TestGenerateAnswer:
    def test_low_score_skips_llm(self):
        """On the cosine-scale path (vector, no rerank), all scores below
        SIMILARITY_THRESHOLD should short-circuit before the LLM call."""
        results = _make_retrieval_results([0.10, 0.05])

        gen_result = generate_answer("What is the policy?", results, search_mode="vector", rerank=False)

        assert gen_result.is_answerable is False
        assert gen_result.confidence == "low"
        assert "could not find" in gen_result.answer.lower()

    def test_low_score_does_not_skip_on_reranked_path(self):
        """The cosine threshold must NOT fire when scores are CE logits.
        Cross-encoder logits routinely go negative for valid matches; comparing
        them against a 0.3 cosine threshold would silently refuse real answers."""
        results = _make_retrieval_results([-0.5, -1.2])  # plausible CE logits
        ...
        with (
            ...
            patch("generator._call_claude", return_value="Real grounded answer.") as mock_claude,
        ):
            gen_result = generate_answer("What is the policy?", results, search_mode="vector", rerank=True)

        mock_claude.assert_called_once()
        assert gen_result.answer == "Real grounded answer."
