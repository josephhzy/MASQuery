...
class TestGetModel:
    ...
    def test_concurrent_calls_construct_once(self):
        """Only one thread should call SentenceTransformer() under concurrent startup."""
        import time
        import threading
        import embeddings
        ...
        # Slow constructor to maximise thread interleaving
        ...
        def worker():
            results.append(embeddings.get_model())

        with patch("embeddings.SentenceTransformer", side_effect=slow_constructor) as mock_cls:
            embeddings._model = None
            ...
            assert mock_cls.call_count == 1, f"Constructor called {mock_cls.call_count} times"
