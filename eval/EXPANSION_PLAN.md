...
**Status: plan not yet executed — second annotator not yet recruited. The existing 13-pair set is v1/single-annotator.**
...
Refusal behaviour is a core feature of MASquery. You cannot measure refusal precision without questions that *should* be refused. Proposed adversarial categories:
...
2. **Fabricated MAS references (3 pairs).** E.g. "What does MAS Notice 999 say about…" — a notice / guideline / circular that sounds plausible but does not exist. Expected outcome: refusal.
3. **Near-miss retrieval (3 pairs).** Questions whose embeddings match a chunk that does *not* answer them. E.g. an Outsourcing question phrased in TRM vocabulary so BM25 fires on the wrong document. Expected outcome: answer from the correct document, or refusal if retrieval fails.
...
Each QA pair goes through a structured process, so that the construction methodology is reproducible and auditable.
...
- **`category`** — already present on the 10 adversarial pairs; to be populated on the standard pairs too: one of `{standard, adversarial_out_of_corpus, adversarial_fabricated, adversarial_near_miss, multi_hop}`
- **`should_refuse`** — already present on the 10 adversarial pairs; to be populated on the standard pairs too: boolean. `true` for adversarial out-of-corpus, fabricated, and near-miss cases — any pair where the correct behaviour is to decline or refuse a specific claim rather than confidently answer; `false` for standard and multi-hop pairs.
