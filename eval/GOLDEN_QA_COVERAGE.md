...
Entries are grouped by source document below — a regrouping of `evaluation/golden_qa.yaml`, not its file order.
...
2. **Adversarial coverage is thin.** 10 adversarial pairs were added (4 out-of-corpus, 3 fabricated MAS references, 3 near-miss) to exercise the refusal path — they produced the historical (pre-fix) Layer-1 refusal numbers in the README; under the current dense+rerank default the Layer-1 score gate is suppressed (by design, 0/10 adversarial pairs are caught at Layer-1 — refusal now relies entirely on Layer-2 LLM detection for these pairs) and refusal is delegated to Layer-2. Still small: refusal recall on 10 items is a coin-flip-away-from-a-coin-flip. Target is 20+ at the scale-up.
