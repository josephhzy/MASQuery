...
Supports three modes: vector-only (default), bm25-only, or hybrid.
...
def _normalize_search_mode(mode: Optional[str]) -> str:
    """Normalize and validate a search mode string."""
    search_mode = (mode or SEARCH_MODE).strip().lower()
    if search_mode not in VALID_SEARCH_MODES:
        valid = ", ".join(VALID_SEARCH_MODES)
        raise ValueError(f"Invalid search mode {search_mode!r}. Must be one of: {valid}.")
    return search_mode
...
def search(
    ...
    - "vector": FAISS cosine similarity; cross-encoder reranked by default (see RERANK_ENABLED)
    - "bm25": BM25 lexical search only
    - "hybrid": BM25 + FAISS combined via Reciprocal Rank Fusion (opt-in)
    ...
    if search_mode == "vector":
        # Over-fetch a k*2 candidate pool when reranking so the cross-encoder can
        # promote chunks that cosine ranked just outside the final top-k (the
        # hybrid branch below already does this). Without it, reranking would only
        # reorder the same k FAISS hits on the default vector+rerank path.
        ranked = _vector_search(query, k * 2 if do_rerank else k)
    elif search_mode == "bm25":
        ranked = _bm25_search(query, k * 2 if do_rerank else k)
    else:
    ...
    results = []
    # For any reranked path (vector+rerank, hybrid+rerank): k*2 is the candidate cap
    # fed to the cross-encoder; the over-fetch already happened above (vector path:
    # _vector_search fetches k*2; hybrid path: both _vector_search and _bm25_search do).
    for rank, (idx, score) in enumerate(ranked[: k * 2 if do_rerank else k], start=1):
        ...
        #   1. The Layer-1 top-score gate in generator.generate_answer is
        #      SUPPRESSED once reranking is on (score_is_cosine is False), so
        #      Layer-1 collapses to the empty-retrieval check — see
        #      docs/REFUSAL_THRESHOLD.md.
        #   2. Layer-2 post-generation refusal detection in generator._detect_refusal
        #      (the primary refusal guard on the default path).
        #
...
def get_indexed_documents() -> List[dict]:
    ...
    Returns:
        List of dicts with name, chunk_count, page_count.
    """
